$(function() {
	...
	function goOperation(orderStatus,orderId,productId,orderComment) 
	{
		...
		{
			//1.删除订单，2。完成评价 
			return '<a href="#"  data-id="'+orderId+'" data-value="'+productId+'" data-comment="'+orderComment+'" class="create-completed">选择操作</a>';
		}
		//交易失败
		if(orderStatus==3)
		{
	...
	$(document).on('click','.create-paid', function (e) {
	      ...
	      var buttons1 = [
	        ...
	        },
	        {
			    ...
			    onClick: function() {
			    	//将买卖双方的订单状态设置为2(已完成)
			    	 var url=operateOrderUrl+'?orderId='+orderId+'&orderStatus=2'+'&productId='+productId;  
		             $.getJSON(url,function(data){
		            	 if(data.success)
		            	 {
		            		 $.alert("收款成功，交易完成！");
